 ...
 * @returns {Path2D} - path object representing a line
 */
...
/**
 * Generates an rectangle shaped path
 * @param {Number} x - x position
 * @param {Number} y - y position
 * @param {Number} w - width
 * @param {Number} h - height
 * @param {String} mode - drawing mode (TL, CENTER) Default: TL
 * @returns {Path2D} - path object representing an ellipse
 */

function rectangle(x, y, w, h, type = "TL") {
  const path = new Path2D(); // inits the path object
  if (type == "CENTER") {
...
export { line, star, ellipse, rectangle };
